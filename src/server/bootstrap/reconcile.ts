...
import { killAkTtydOrphans } from "../adapters/ttyd.js";
 ...
 * @remarks IN-01 compares the PERSISTED session name (derived `ak-<identifier>` only as fallback);
...
export async function reconcileSessions(): Promise<void> {
  ...
  for (const card of store.cardsWithSession()) {
    ...
    const sessionName = card.tmuxSession ?? "ak-" + card.identifier;
    if (!live.has(sessionName)) {
      await store.markSessionLost(card.id);
      lost++;
    }
  }
  const killed = await killAkTtydOrphans();
  console.log(
