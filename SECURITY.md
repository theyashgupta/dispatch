...
Dispatch is **one user, one machine, localhost only.** Everything binds to `127.0.0.1`, and there is
**no authentication layer, on purpose.** ttyd hands out a shell attached to a live agent session; the
...
Dispatch is pre-1.0 and moves fast. Only the latest `main` is supported; please confirm an issue against
