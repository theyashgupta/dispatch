...
4. **Persistence format + location.** `~/.dispatch/{board.json,config.json}`; `board.json` ===
   ...
   and group cards never get that section (`T-99-04`).
...
| T-99-01 | Tampering / Information Disclosure      | `routes/cards.route.ts` (`GET /cards/:id/attachments/:name`), `services/domain/attachments.ts`                                                         | Both params are regex-validated (`CARD_ID_RE` = `^[A-Za-z0-9_-]+$`, `ATTACHMENT_NAME_RE` = `^[a-f0-9]{16}\.(png                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | jpg | gif | webp)$`) before `res.sendFile(name, { root: attachmentsDir(id), dotfiles: "deny" })`, so no request can name a file outside the card's own attachment folder; an encoded `..%2F`in either param is a 400, never a lookup. The route lives on`apiRouter` and inherits the single loopback / remote-auth gate. |
| T-99-02 | Tampering                               | `services/domain/attachments.ts` (`imageExtension`, `decodeImages`)                                                                                    | Pasted image bytes are typed by magic bytes only (PNG, JPEG, GIF, WebP; never SVG), and the on-disk name is the SHA-256 prefix plus the detected extension. The client-declared MIME type and filename are never read, so nothing user-controlled reaches the filesystem path or the served `Content-Type`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| T-99-03 | Denial of Service                       | `services/domain/attachments.ts` (`decodeImages`), `bootstrap/index.ts` (`ATTACHMENT_BODY_LIMIT`)                                                      | At most `MAX_ATTACHMENTS` (10) images of `MAX_ATTACHMENT_BYTES` (10 MiB) per request; the base64 string length is bounded before decoding so an oversize image is never decoded into a Buffer. Only `POST /api/cards` and `POST /api/cards/draft` get the larger JSON window (`ATTACHMENT_BODY_LIMIT`, derived from those two constants plus base64 overhead); every other `/api` route keeps the 1mb parser.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| T-99-04 | Tampering                               | `services/domain/kickoff.ts`, `services/domain/attachments.ts` (`attachmentLinks`, `withAbsoluteAttachments`)                                          | The kickoff's `## Attached images` section and the rewritten description links are built only from `attachments/<16-hex>.<ext>` matches in a local card's description, joined to the server-derived `attachmentsDir(card.id)`; Linear-sourced and group cards never get the section. No free text from the description can become a line of that section, so it cannot carry a spoofed `DISPATCH_STATUS:` marker.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
