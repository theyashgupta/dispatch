...
| NEW-21  | Information Disclosure                  | `store/board.store.ts` (`redactCard`)                                                                                                                  | A session copy riding the wire unstripped would be a credential leak: `redactCard` now also `delete`s `wireCard.sessions` outright (the full array is server-side only and carries every session's `hookToken`) and builds `wireCard.activeSession` by FIELD-PICKING the six `ActiveSessionWire` keys rather than spreading the resolved session, so the secret is omitted by construction and a future field added to `Session` cannot leak through this path. Still the SAME single sanctioned redaction site (`NEW-21`'s own chokepoint) — proven by a repo-wide grep asserting exactly one `delete …hookToken` line exists, and a capture-and-grep on a real `activeSession` payload that is itself proven able to fail by a deliberate unredaction. |
