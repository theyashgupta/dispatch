...
- Cross-Module Invariants
  ...
  - [Unwind and Archive](#unwind-and-archive)
...
session running the user's login shell, types the Claude REPL launch into that shell, and spawns
a per-session ttyd so the terminal can be embedded in the detail-panel iframe. A pane watcher samples the tmux transcript every couple of
...
**One decision reads one wire field, by construction, not by convention (Phase 102).** Before this
...
`repairDowngradeDrift` (below) still guards a real, separate disagreement between the flat mirror
...
requiring the two to be mechanically generated from one another. Eighteen triggers (the Parked drag row below restates #8 for its own column), against their
...
| Marker while Parked (LOCAL-17)                               | `MARKER_CONSUMED_SOURCES` (`parked`)                                             | column-preserving (key recorded)                 | `board.store.ts#applyMarker` consume branch                                               |
...
creates a fresh login-shell tmux session (only when none is live) and launches
`claude --resume <id>` (or `--continue`) into it through the same `launchClaude` helper the start
saga uses, with NO kickoff prompt re-sent. Its store
...
**Parked is set aside, not live (LOCAL-17).** `parked` is a seventh column between In Review and Done
for work that exists but nobody is driving. It has NO automatic in-edge: only a manual drag or the
Move-to picker lands a card there. While a card is Parked, `applyMarker` CONSUMES a marker rather than
applying it: the key is recorded on the session (and the card mirror) exactly as on a move, but the
column, `statusReason`, the member mirror and the activity event are all skipped
(`shared/column-transitions.ts#MARKER_CONSUMED_SOURCES`). Recording is what makes the single
automatic out-edge safe: a `UserPromptSubmit` (or pause-tool `PostToolUse`) flips a Parked card to In
Progress through `flipBack`, and because `parked` is NOT in `FLIP_BACK_CLEARS_LAST_MARKER` the consumed
key survives the flip and dedups the marker text still on the pane. Excluding Parked from
`applyMarker` outright would drop the key and let the level-triggered scan re-apply the marker the
instant the card re-entered In Progress. Everything else Parked inherits from the exclusion-based
guards unchanged: session-lost detection and Resume stay on, `/start` is not blocked, and cleanup
teardown remains Done-only, so Parked to Done stamps the schedule and Done to Parked clears it through
`moveCardManual` with no special case.
...
session the terminal region is exactly one `<iframe src="/sessions/${activeSessionId}/terminal/">`
...
- **The left-edge resize handle's drag shields the iframe with a drag-duration overlay, not
  ...
  `/sessions/${activeSessionId}/terminal/` path — `@see` [Terminal (ttyd)](#terminal-ttyd) — but
...
already re-pointed by id. Keying the iframe on `activeSessionId` was the phase's original design and
...
reversal was correct by measurement, not by argument: with `key={c.activeSessionId}` temporarily
...
board interactions" rule. A 409 (the pane is not at its shell prompt, so nothing was typed) is
logged to the console only, exactly like the other fire-and-forget calls; the person can see the
running program in the pane.

**Where it renders.** Only inside the iframe branch: a card with a live session, a ttyd port, and
no `terminalError`. The session-lost state renders `SessionLostSection` (Resume, Restart) instead,
and the ttyd-died state renders the Reconnect notice, so the button never appears where there is
no shell to type into. The iframe keeps its identity across a click (no remount, scrollback stays);
only the pane's foreground process changes.
...
**The pane runs the login shell; claude is a program typed into it (`SHELL-01`).** `newSession` is
called with an EMPTY command argv, so tmux runs its `default-shell` as a login shell (resolved by
tmux from the server's `SHELL`, then the passwd entry, then `/bin/sh`; dispatch resolves nothing
itself, which is what keeps a launchd-started service on the user's real shell). Both the start saga
and Resume go through one helper, `steps.ts#launchClaude`: create the session ONLY when `hasSession`
says it is absent (a live session is reused and never killed), wait until the pane's root shell
(`#{pane_pid}`) owns the terminal's foreground process group for two consecutive polls
(`adapters/tmux.ts#paneAtPrompt`, `ps -o pgid=,tpgid=`; names are never compared, because tmux
reports the exec'd image while `ps` reports argv0, and a fresh pane reads idle for a few tens of
milliseconds before the rc's first child takes the tty), then type the launch as ONE fully quoted
command line with `send-keys -l`, submit it with a separate `send-keys Enter`, and poll
`awaitReplReady` as before. The wait is the moment the rc files have released the terminal: a
startup prompt such as an ssh-agent PIN would otherwise swallow the typed line as its own input, and
an rc that never reaches its prompt fails the start with that prompt visible in the captured pane.
The pty shim wraps the typed line, not the shell (`wrapWithPtyShim` is a no-op on an empty argv), so
claude's frames are still stripped while `git status` at the prompt runs plain. A Ctrl-C or `/exit`
in claude therefore returns the pane to a shell prompt in the worktree instead of ending the tmux
session: the 3-strike capture-failure detector, `markSessionLost`, `reconnectTerminal` and the
Resume affordance keep their one meaning, "the tmux session itself is gone". The wait recognises a
prompt implemented by a separate process (the measured `ssh-add` case); a prompt written as a shell
builtin (`read`, `vared`) runs inside the shell's own process group and reads as idle, so its input
swallows the line and the start fails as a `repl-timeout` with that pane visible, the honest outcome
for a rc Dispatch cannot see into. A card whose claude has exited stays in its column with a live
terminal and no status source until Run Claude or a hand-typed launch, by the ticket's decision that
a claude exit is not a session loss; showing "claude exited" on the card is a later ticket. Proven
by `steps.test.ts` against a private tmux server with a fake `claude`: Ctrl-C leaves the session
alive at the login shell, and a second `launchClaude` reuses it.

**The launch line is single-quoted, and that is the shell boundary (`SHELL-01`, `T-04-01`).** The
line goes through `services/domain/claude-launch.ts#shellQuote`, which single-quotes every token (a
literal `'` becomes `'\''`) so the resolved claude path, the resume flags, the hooks settings path,
and every Settings `claudeArgs` token arrive in claude as exactly one argument each. This is the ONE
place dispatch hands text to a shell, and it holds for POSIX shells (measured in sh, bash and zsh);
fish is not POSIX and can mis-parse that escape for a token holding a backslash next to a quote,
which no reachable token can contain. Quoting protects against the shell's PARSER only: the line is
delivered through the line editor, which acts on control bytes before the parser sees a quote
(`\x15` erases the trusted prefix, `\x04` ends the login shell and with it the session, a TAB
completes), so `shellQuote` refuses any token carrying a C0 control or DEL (newline excepted, since
an open quote carries it onto a continuation prompt as text), and `PUT /config/claude-args` rejects
such strings at the door. Before the line is typed, `typeLaunchLine` sends `C-u` and `C-l`: the
first discards anything half-typed at the prompt, the second clears the screen so `awaitReplReady`
cannot match a READY footer left on screen by the previous claude run; both are line-editor keys and
never land in history. Because the typed line itself does land in the shell's history, Up-arrow then
Enter relaunches claude with the same flags.

**What the created session's `-e` env does and does not cover (`SHELL-01`).** The hook token and
`CLAUDE_CONFIG_DIR` reach the shell solely through `new-session -e`, so `launchClaude` mints a token
only for a session it creates; a reused session keeps the token its shell already holds
(`steps.ts#existingHooks`), and a relaunch into a live session types a line with no env at all.
Minting on reuse would register a token the shell can never see and revoke the one it has, which is
a silent loss of the whole status channel. The same `-e` carries `DISPATCH_SHELL_SESSION=1`, the
marker that says "this pane's root is the login shell": `runClaude` checks it with
`show-environment` and refuses (`legacy`, 409) a session created by an older build, whose pane root
is claude itself and would take the typed line as prompt input; such a session becomes a shell
session naturally when claude exits and Resume relaunches it. The token is visible inside the ticket
shell (`echo $DISPATCH_HOOK_TOKEN`) by construction, accepted because the shell is the same
person's, on the same machine, with the same terminal access the token gates; it never enters argv,
the typed line, logs or wire snapshots. Session creation reports itself through `onCreated` the
instant `new-session` returns, before the two readiness waits, so the start saga's
`tmuxSessionCreated` and Resume's `killTarget` cover a launch that later times out; a rollback
therefore still kills the shell it created and never one it reused.

**Run Claude is a server relaunch into the live shell (`SHELL-01`).** `POST
/api/cards/:id/run-claude` (`routes/cards.route.ts`: unknown card 400, grouped member 409, no live
session 400) calls `services/orchestration/run-claude.ts#runClaude`, which answers `busy` (409)
unless `paneAtPrompt` says the pane's root shell owns the foreground and no start or relaunch of the
same card is in flight (a running claude, a pager, a rc prompt, or a second `send-keys -l`
interleaving on one shell line is never typed into), `no-session` (400) when the tmux server no
longer has the session (the seconds before the watcher marks it lost), `legacy` (409) when the
session lacks the `DISPATCH_SHELL_SESSION` marker, and `account` (409) when the session's recorded
Claude account no longer resolves, so the handler never falls through to Express's HTML 500.
Otherwise it types the same launch line a first start would, through `steps.ts#typeLaunchLine`, with
`--resume <claudeSessionId>` when the hooks channel recorded the conversation id and a NEW
...
[-e KEY=VALUE ...]`. The explicit `-x 200 -y 50`
...
containing spaces survives as one token; this is quote-aware tokenizing only, never shell
interpretation. The tokens are then re-quoted by `shellQuote` before the launch line is typed into
the ticket's login shell (`SHELL-01`), so a token still cannot glob, expand `$VAR`, or chain
commands: it reaches claude as one literal argument. Both session-start (`services/orchestration/steps.ts`)
...
Unwind (LOCAL-17) is the only path that takes a started group apart, and it is reversible by
construction. `services/orchestration/unwind.ts#unwindGroup` resolves the group from either the
group card or any member, refuses with 409 while a start, resume or cleanup saga holds the card
(`isStarting` covers resume, which runs inside the start bracket), kills every session's ttyd and
tmux through `cleanup.ts#killSessionProcesses`, and only then hands the store one mutation.
`board.store.ts#unwindGroup` clears every session through the same `loseSession` body
`markSessionLost` uses (so the six flat fields still have exactly one writer), snapshots the whole
card into the `archive` table, deletes the group card from the live map, and unlinks each member into
the chosen destination. Worktrees, branches and the workspace folder are never touched by Unwind.

**Restore is all-or-nothing.** `board.store.ts#restoreGroup` consults the pure `restoreBlocker`:
every member must still exist, be ungrouped, carry no session history, and sit in the destination
Unwind chose, and the group id must not be live. Any blocker refuses with the member named and
changes nothing. On success the snapshot card is re-inserted as a whole object, the boot-hydration
shape, so no flat session field is assigned; every session is already `sessionLost`, which is what
makes the existing Resume affordance the recovery path with no re-setup.

**Hard delete is the only destructive step, and it is explicit or aged.**
`services/orchestration/archive-delete.ts#deleteArchivedGroup` reuses the Done cleanup saga's
extracted steps (`cleanup.ts#dirtyRepos`, `#removeWorkspaceFiles`) per workspace the archived card
owns, in the locked `NEW-14` order, and branches always survive. Without `force` the dirty-worktree
preflight refuses and records the reason on the row so the UI can offer Delete anyway. The cleanup
scheduler's tick runs `cleanup-scheduler.ts#runArchiveSweep`, which deletes rows older than
`archiveRetentionDays` (config, default 30, 0 means never) with `force` false; a blocked or failed
...
**A hand-typed bare `claude` is hook-silent by decision (`SHELL-01`).** The hooks layer reaches
claude only through the `--settings <hooks file>` flag on the launch line, while the hook script's
own `DISPATCH_*` env gate is satisfied by the tmux session environment the shell inherited. A person
who types plain `claude` at the ticket's shell prompt therefore starts a claude that fires no hook
events; under `statusChannel: auto` the pane channel is already demoted for that session
(`hookRoutedAt` stamped), so the card's status stays where it was until the next full launch. This
is accepted rather than patched: the full launch line sits in the shell history (Up-arrow, Enter)
and the terminal's Run Claude button always uses it.
...
exemptions: `tokens.css` itself, the palette's single legitimate home, and the `--hl-*` custom
...
4. **Persistence format + location.** `~/.dispatch/{board.json,config.json}`; `board.json` ===
   ...
   (folder `0700`, files `0600`), referenced from the description as `attachments/<name>` and served
...
<quoted line>` followed by a separate `send-keys Enter`; `capture-pane -p -J -t =<name>:`; exact-name
...
disableLeaveAlert=true -t DISPATCH_TTYD_REVISION_<revision>=1 tmux -u attach -t =<session>` (`-u` is
   ...
   adoption/spare (the sole re-adoption fingerprint); iframe src `/sessions/${activeSessionId}/terminal/`
...
| T-04-01  | Tampering / Elevation                             | `adapters/markers/watcher.ts`, `adapters/exec.ts`, `adapters/gh.ts`                                                                                                                                              | Session names entering `capture-pane -t =<name>` are `dsp-` + a route-validated identifier and travel argv-only via `run()`; captured pane text is inert stdout, never a command. The one shell surface, the claude launch line typed into the ticket's login shell, is built from server-resolved tokens only (claude path, fixed flags, hooks settings path, Settings `claudeArgs`) and every token is single-quoted by `services/domain/claude-launch.ts#shellQuote` before `send-keys -l`, so no token can expand, glob, or chain, and any token carrying a control byte (which the line editor would act on before the parser) is refused (`SHELL-01`). The `gh pr list` PR probe travels argv-only through the same `run()` chokepoint, with only the server-derived `card.branch` and the registered `card.workspace.repos[].path` entering argv — never a client-supplied value.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| NEW-21   | Information Disclosure                            | `store/board.store.ts` (`redactCard`)                                                                                                                                                                            | A session copy riding the wire unstripped would be a credential leak: `redactCard` `delete`s `wireCard.sessions` outright (the full array is server-side only and carries every session's `hookToken`), so the secret is omitted by construction and a future field added to `Session` cannot leak through this path. The active session's own `ttydPort`/`activeSessionId` already ride the wire unconditionally via the card's own flat mirror fields (`setActiveSession`'s chokepoint), so `redactCard` builds no separate active-session wire object at all (Phase 102 deleted the nested field-pick, `96-11`'s F-96-F fix having already narrowed it to two keys as an interim step). Still the SAME single sanctioned redaction site (`NEW-21`'s own chokepoint) — proven by a repo-wide grep asserting exactly one `delete …hookToken` line exists, and a capture-and-grep on the real wire payload that is itself proven able to fail by a deliberate unredaction.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| T-103-01 | Information Disclosure                            | `services/domain/vault.ts#listKeys`, `routes/vault.route.ts`, `shared/types.ts#VaultKeySummary`                                                                                                                  | The list path opens only `vault.json` and never `values.env`; `VaultKeySummary` is a closed six-field shape (name, purpose, createdAt, updatedAt, filled, hasPrevious) with no value field and no length-proportional field, so the listing can never carry a value or a size hint structurally. Exactly two single-key reads carry a value, each on explicit request only: `GET /vault/:name/value` (`services/domain/vault.ts#readCurrent`, the outgoing value the rotate flow shows before it is replaced) and `GET /vault/:name/previous` (`services/domain/vault.ts#readPrevious`, the single value each key held before its latest rotate, moved verbatim into the sealed `previous.env` by `setValue`); the guard seals `previous.env` exactly as it seals `values.env` (`T-105-04`). The `vault-no-read-back` check sweeps seven read-back path shapes plus the board payload plus live SSE frames plus all four error paths for a seeded sentinel, and was proven able to fail by temporarily adding a read-back route.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| T-103-02 | Tampering                                         | `services/domain/vault.ts#writeStore`                                                                                                                                                                            | Every write is `write-file-atomic` at mode `0o600` followed by an unconditional `chmodSync`, and the containing directory is `chmodSync`ed to `0o700` on every mutating call, since `mkdir`'s `mode` option is create-only and a no-op on an existing directory. Observed (103-03): the directory-level re-assert is load-bearing (a hand-loosened `0755` directory is never re-tightened without it); the per-file `values.env` re-assert was observed NOT load-bearing on this machine, since `write-file-atomic`'s rename-over-target already discards a hand-loosened inode, and is kept as defence-in-depth. Delete matches a value line by a `NAME=` prefix (not a bare name), so deleting `FOO` never touches `FOOBAR`; every value is POSIX single-quote-escaped so it stays safe for a later `. values.env` source.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| T-105-01 | Information Disclosure                            | `bootstrap/hook-setup.ts` (`VAULT_RUN_SCRIPT`, the generated `~/.dispatch/vault-run` runner)                                                                                                                     | The generated runner sources `values.env` wholesale (`set -a; . "$VALUES"; set +a`), then walks the schema file line by line and unsets every name not present in the caller's own `--keys` list, so only the requested keys survive into the wrapped command's environment; the wrapped command is then `exec`'d as a replacing child process, so the invoking shell itself is never mutated. `vault-run-keys` proves the narrowing load-bearing via a relaxed scratch copy with the unset pass disabled: `leg 4: relaxed copy observed to leak RUN_KEY_TWO's sentinel, the narrowing assertion (leg 1/leg 3) is load-bearing`, with the real artifact's hash unchanged across the break (105-05-SUMMARY.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| T-105-02 | Information Disclosure                            | `bootstrap/hook-setup.ts` (`VAULT_RUN_SCRIPT`, the runner's refusal gate)                                                                                                                                        | Six dumper basenames (`env`/`printenv`/`set`/`export`/`declare`/`typeset`), a never-created or unfilled requested key, and malformed invocation shapes each refuse non-zero with a message naming Settings, Vault, and every decision is taken BEFORE `values.env` is ever sourced, so a refused invocation never had a value in its own process environment. Both the `--keys` list and the schema's own name column are matched against the same `[A-Z_][A-Z0-9_]*` shape the store validates on create, so the later unset loop can never act on an attacker-supplied token. `vault-run-refusals` proves two break legs falsifiable: `break A: relaxed copy exit 0 (dumped) vs real artifact exit 2 (refused)` and `break B: relaxed copy exit 0 (let it through, RUN_KEY_THREE unset) vs real artifact exit 3 (refused)` (105-05-SUMMARY.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| T-105-03 | Information Disclosure                            | the generated hook settings' `permissions.deny` Read rule (`hookSettingsJson()` in `bootstrap/hook-setup.ts`)                                                                                                    | ACCEPTED, best-effort defense-in-depth, not proven to hold alone on the currently installed CLI: one exact double-leading-slash `Read(//<abs values path>)` entry, no glob, covering the CLI's own Read tool and (per 105-01's spike) a Bash `cat` naming the literal path as an argument. 105-01's `claude -p` spike (CLI 2.1.241) observed this rule block both vectors alone; a real interactive `--dangerously-skip-permissions` session on CLI 2.1.245 (`vault-bypass-guards`, 105-07) observed the SAME deny-only rule LEAK the sentinel through the Read tool, a CLI permission-engine behavior change rather than a Dispatch regression (the rule's own generation and rendered text were unchanged and independently confirmed present in the derived variant). This rule ships as defense-in-depth alongside the load-bearing PreToolUse guard (`T-105-04`), never as an independently sufficient control on the current CLI. See Known Residuals.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| T-105-04 | Information Disclosure                            | the generated Bash `PreToolUse` guard, a Node ESM script written and cited by `bootstrap/hook-setup.ts` (never referenced by its generated filename, which lives under the user's home directory, not this repo) | Ports `env-vault-guard.py`'s deny classes (read-out, copy/exfiltration, raw source) plus a runner-mediated-dump check with no analog in the prototype, deny-broadly on any ambiguous `vault-run` invocation shape, and allows file management (`ls`/`stat`/`chmod`/`open`/`code`/`file`/`du`) plus the schema surface the kickoff block teaches sessions to read directly. `vault-guard-decisions` drives the guard's full 31-row deny/allow table with an in-run falsifiability leg: `falsifiability leg: relaxed copy observed to allow the read-out row, proving the READ_OUT deny class load-bearing` (105-06-SUMMARY.md). On a real interactive bypass-permissions session (`vault-bypass-guards`, 105-07), the SHIPPED configuration (this guard plus `T-105-03` together, exactly what boots) blocked both the Read-tool and Bash vectors with verbatim refusal evidence: `vault-guard: unrecognized operation ... Everything else on this file is blocked`. This guard is therefore the LOAD-BEARING enforcement layer for VLT-08 on the current CLI, since `T-105-03` alone was observed to leak. Plan 02's own one bounded `vault-audit` run (107-02, 2026-08-26) widened the live catalogue to 12 guard-and-deny-governed vectors (Read, cat, head, tail, interpreter `open()`, cp, curl, source, dot-source, chained `ls && cat`, case-varied basename, relative-path-after-cd) and observed all 12 BLOCKED under the shipped `both` configuration and all 12 LEAKED under `neither`, the falsifiability control now fully captured rather than proxied. Two mechanism-distinct both-only vectors (runner-mediated dump, the nonexistent HTTP read-back route) scored INCONCLUSIVE on Opus response latency exceeding the raised 180s settle budget, a named coverage residual, not a leak. The contract-follow leg's own two assertions (running a legitimate wrapped command that echoes its own injected env; a final direct-read probe) also scored non-blocked: the real session self-censored both, declining to run the reporter script or attempt the read at all, reasoning either action resembled exfiltration even though the runner's own denylist would have permitted the former (`T-105-05`'s own accepted residual) and the guard would have technically refused the latter; over-caution, never a leak, but a measurement gap the marker-based scorer could not resolve into a captured `blocked` state (`107-vault-audit-run.log`). The guard-ONLY isolation leg (guard active, deny rule removed) remains uncaptured for this widened catalogue, by design (Plan 02 cost-bound the live matrix to `both`/`neither` only, reusing 105's own already-characterized `deny-only`/`guard-only` legs for the original two-vector case); a future run that completes it for the full catalogue would close this residual rather than merely narrow it. |
| T-105-05 | Information Disclosure                            | the wrapped command's own process environment                                                                                                                                                                    | ACCEPTED, not mitigated: a session that legitimately invokes the runner receives the injected values as that command's own environment, and any child it spawns can read its own environment; this is inherent to inject-into-the-process-being-run and is the same boundary every comparable run wrapper accepts. Bounded, not total: `vault-residual-demo` measures three bounds live, the invoking shell carries neither name before or after the wrapped command runs, the unrequested key is absent from the wrapped command's own output, and that output is process-environment-shaped, never the values file's own quoted line, so the file itself is never read. Quoted verbatim from the check's own output: `RESIDUAL: a command legitimately wrapped by vault-run can print its own injected environment to itself or a child it spawns; the invoking shell and any unrequested key never see it. Inherent to per-command injection, accepted, never closed.` (105-06-SUMMARY.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| T-105-06 | Information Disclosure                            | the launch condition in `orchestration/steps.ts` (`startClaude`) that decides whether the `--settings` flag carrying `T-105-03` and `T-105-04` is passed at all                                                  | ACCEPTED, inherited, not introduced here: a session launched below the hooks-capability floor (`checkHooksCapability`, `bootstrap/hook-setup.ts`), under a forced pane status channel (`runtime.statusChannel === "pane"`), or with hooks env-disabled (`DISPATCH_HOOKS_DISABLED=1`) receives no settings flag at all, so BOTH structural layers are absent and the runner's own refusal gate (`T-105-01`, `T-105-02`) is the only vault protection such a session has. This degrade path pre-dates this phase; VLT-08's guarantee is therefore conditional on hooks capability, not universal. Closest available proxy: 105-01's `claude -p` spike's `neither` cell (no Dispatch mechanism active) leaked both vectors, Read returned the raw sentinel line, Bash `cat` returned the same with no denial, the falsifiability control this row's absence-of-protection claim rests on. Plan 02's one bounded `vault-audit` run (107-02, 2026-08-26) captured the widened `neither` cell directly: all 12 guard-and-deny-governed vectors leaked the sentinel with neither mechanism active, closing the coverage gap 105-07 left open for the falsifiability control itself (`vault-bypass-guards` real evidence now corroborates, not merely proxies, 105-01's spike). This row's own below-hooks-floor structural gap is unaffected by that capture: a session launched below the hooks-capability floor still receives no `--settings` flag at all regardless, an inherited condition no live-session run can observe directly since no such session is ever spawned by this check. See Known Residuals.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| T-116-01 | Tampering / Information Disclosure                | `routes/cards.route.ts` (`GET /cards/:id/attachments/:name`), `services/domain/attachments.ts`                                                                                                                   | Both params are regex-validated (`CARD_ID_RE` = `^[A-Za-z0-9_-]+$`, `ATTACHMENT_NAME_RE` = `^[a-f0-9]{16}\.(png                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | jpg | gif | webp)$`) before `res.sendFile(name, { root: attachmentsDir(id), dotfiles: "deny" })`, so no request can name a file outside the card's own attachment folder; an encoded `..%2F`in either param is a 400, never a lookup. The route lives on`apiRouter` and inherits the single loopback / remote-auth gate. |
| T-116-02 | Tampering                                         | `services/domain/attachments.ts` (`imageExtension`, `decodeImages`)                                                                                                                                              | Pasted image bytes are typed by magic bytes only (PNG, JPEG, GIF, WebP; never SVG), and the on-disk name is the SHA-256 prefix plus the detected extension. The client-declared MIME type and filename are never read, so nothing user-controlled reaches the filesystem path or the served `Content-Type`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| T-116-03 | Denial of Service                                 | `services/domain/attachments.ts` (`decodeImages`), `bootstrap/index.ts` (`ATTACHMENT_BODY_LIMIT`)                                                                                                                | At most `MAX_ATTACHMENTS` (10) images of `MAX_ATTACHMENT_BYTES` (10 MiB) per request; the base64 string length is bounded before decoding so an oversize image is never decoded into a Buffer. Only `POST /api/cards` and `POST /api/cards/draft` get the larger JSON window (`ATTACHMENT_BODY_LIMIT`, derived from those two constants plus base64 overhead); every other `/api` route keeps the 1mb parser.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| T-116-04 | Tampering                                         | `services/domain/kickoff.ts`, `services/domain/attachments.ts` (`attachmentLinks`, `withAbsoluteAttachments`)                                                                                                    | The kickoff's `## Attached images` section and the rewritten description links are built only from `attachments/<16-hex>.<ext>` matches in a local card's description, joined to the server-derived `attachmentsDir(card.id)`; Linear-sourced and group cards never get the section. No free text from the description can become a line of that section, so it cannot carry a spoofed `DISPATCH_STATUS:` marker.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| T-L17-01 | Tampering / Elevation                             | `routes/archive.route.ts`, `routes/cards.route.ts` (`/unwind`), `services/orchestration/archive-delete.ts`, `services/orchestration/cleanup-scheduler.ts` (`runArchiveSweep`)                                    | Every filesystem and subprocess target of Unwind, Restore and archive Delete is read from the stored archive row (session `workspacePath`, `workspace.repos`, `tmuxSession`), never from a request body, which carries only an id, a two-value `to` and a boolean `force`; ids resolve through the store, unknown ids 404. Delete refuses while the group id is live again (`restored`), while another delete holds the card-scoped cleanup bracket (`busy`), and without `force` whenever the dirty-worktree preflight reports uncommitted work or cannot check a worktree; branches are never deleted. The retention sweep is the first server-initiated destructive filesystem path: it runs with `force` false only, skips rows whose group is live, and treats a blocked or failed row as terminal. Wire responses pass through `board.store.ts#redactArchivedGroup`, so no card snapshot, session record or hook token leaves the process.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
### `permissions.deny` is best-effort defense-in-depth, not a confirmed independent layer (`T-105-03`)

105-01's pre-flight spike (`claude -p`, CLI 2.1.241) observed the generated settings'
double-leading-slash `permissions.deny` Read rule (`T-105-03`) block a real bypass-permissions
probe alone, on both the Read tool and a Bash `cat` naming the literal path. A later real,
interactive `--dangerously-skip-permissions` session on CLI 2.1.245 (`vault-bypass-guards`,
105-07) observed the SAME rule, unchanged, LEAK the sentinel through the Read tool when run
without the PreToolUse guard present. Neither the rule's generation nor its rendered text
regressed between the two runs; the installed CLI's own permission-engine behavior changed, or
diverges between headless `-p` and an interactive tmux session, between versions. The vault-guard
`PreToolUse` Bash hook (`T-105-04`) is therefore the LOAD-BEARING enforcement layer for VLT-08 on
the current CLI. `permissions.deny` remains in the generated settings as defense-in-depth, still
governing the Read tool where the guard cannot reach, and it is one of three layers protecting the
real shipped `values.env` path (the guard, this rule, and, for this operator specifically, a
personal global `~/.claude/CLAUDE.md` `*.env` rule that refuses on filename before either Dispatch
mechanism is ever reached, see `105-07-SUMMARY.md`'s Criterion 3 disposition), but it is not, by
itself, confirmed sufficient. The guard-only isolation leg (guard active, deny rule removed) was
not captured by any run to date; a future run that completes it would close this residual rather
than merely narrow it. VLT-08 is marked complete on the narrowed claim the evidence actually
supports: the layered, shipped configuration blocks a real bypass-mode session, proven;
`permissions.deny` alone does not, on the currently installed CLI.

### A launch below the hooks-capability floor carries neither structural vault layer (`T-105-06`)

`orchestration/steps.ts`'s `startClaude` step passes the `--settings` flag (and therefore both
`T-105-03` and `T-105-04`) only when the installed CLI clears the hooks-capability floor and the
session is not forced onto the pane status channel; `DISPATCH_HOOKS_DISABLED=1` forces the same
below-floor branch deliberately, for smoke runs. A session launched under any of these three
conditions receives no settings flag at all, so both the `permissions.deny` rule and the
PreToolUse guard are structurally absent for that session, not merely weakened, leaving the
runner's own refuse-before-sourcing gate (`T-105-01`, `T-105-02`) as the only vault protection
such a session has. This degrade path pre-dates this phase: it is the same below-floor/pane-forced
branch the hooks feature has always had, not a new gap this phase introduced, and it means VLT-08's
guarantee is conditional on hooks capability rather than universal. Cross-references `T-105-06`.

## Verification Gates

Dev tooling plus one small `node:test` suite. Each gate answers one narrow, mechanically-checkable
question; none of them read prose for truth.
...
- **`node scripts/panel-99.mjs`** (`npm run panel-99`, Phase 99, PORT-02) - a CDP structural/DOM
  ...
  the evidence surface changes no rendered card geometry. Card geometry is not the whole DOM
  contract, though: this phase also changed the badge's accessible NAME, from `Open preview` plus an
  em dash plus `localhost:<port>` to the comma-separated `Open preview, localhost:<port>`, when the
