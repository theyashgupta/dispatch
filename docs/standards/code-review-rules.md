...
- **The 3 TODO-57 warn edges.** `lib/card-badges.ts -> hooks/useUnseenActivity` and `primitives/ActivityItem.tsx -> lib/event-copy` / `lib/format-age` are genuine layering violations, kept at `warn` (not `error`, not silenced) via a named trailing carve-out block in `eslint.config.ts`, pointing at `docs/standards/architecture.md`'s "Triage-derived layering-violation fixes" gap-list entry. These are temporary until Phase 57 relocates/hoists the offending calls — flag them in review as known, tracked debt, not as new findings.
