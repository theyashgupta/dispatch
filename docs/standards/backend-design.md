...
| **State (store)**            | `store/board.store.ts` (+ `store/mapping.ts`)                                                                             | The single writer of board state: a mutation queue is the only path that changes `board.json` or the in-memory snapshot.             |
