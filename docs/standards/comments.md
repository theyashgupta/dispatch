...
| Cross-module invariant / protocol / trap (single-writer store, `AK_STATUS`/`⏺` marker protocol, tmux `-l` literal-send trap, PANEL-03 no-remount rule, loopback/DNS-rebinding threat model) | `docs/ARCHITECTURE.md` with a stable anchor, referenced from JSDoc via a short `@see docs/ARCHITECTURE.md#anchor` pointer |
